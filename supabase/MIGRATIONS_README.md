...
`migrations/20260614040000_apply_pending_live_sync.sql` consolidates
the three pending migrations into a single, fully idempotent SQL file
in dependency-safe order. It is the one-time remediation that catches
the live DB up to the state GitHub thinks it is in.
...
3. Paste the entire contents of
   `migrations/20260614040000_apply_pending_live_sync.sql`.
4. Click **Run**.
